...
pub type PacketBytes = []u8

// ====== POP ======

pub fn (mut bytes PacketBytes) pop_u8() u8 {
	return bytes.pop_left()
}

pub fn (mut bytes PacketBytes) pop_u16() u16 {
	return binary.big_endian_u16([bytes.pop_left(), bytes.pop_left()])
}

pub fn (mut bytes PacketBytes) pop_u32() u32 {
	return binary.big_endian_u32([bytes.pop_left(), bytes.pop_left(), bytes.pop_left(), bytes.pop_left()])
}

pub fn (mut bytes PacketBytes) pop_u64() u64 {
...
pub fn (mut bytes PacketBytes) pop_i8() i8 {
	return i8(u8(bytes.pop_u8()) + u8(min_i8))
}

pub fn (mut bytes PacketBytes) pop_i16() i16 {
	return i16(u16(bytes.pop_u16()) + u16(min_i16))
}

pub fn (mut bytes PacketBytes) pop_i32() i32 {
	return i32(u32(bytes.pop_u32()) + u32(min_i32))
}

pub fn (mut bytes PacketBytes) pop_int() int {
	return int(bytes.pop_i32())
}

pub fn (mut bytes PacketBytes) pop_i64() i64 {
	return i64(u64(bytes.pop_u64()) + u64(min_i64))
}

pub fn (mut bytes PacketBytes) pop_bool() bool {
	return bytes.pop_u8() != 0
}

pub fn (mut bytes PacketBytes) pop_f32() f32 {
	return unsafe { U32_F32{u: bytes.pop_u32()}.f }
}

pub fn (mut bytes PacketBytes) pop_f64() f64 {
	return unsafe { U64_F64{u: bytes.pop_u64()}.f }
}

pub fn (mut bytes PacketBytes) pop_string() string {
	return bytes.pop_arr[u8]().bytestr()
}

pub fn (mut bytes PacketBytes) pop_color() Color {
	return Color.rgba8( bytes.pop_u8(), bytes.pop_u8(), bytes.pop_u8(), bytes.pop_u8() )
}

pub fn (mut bytes PacketBytes) pop_vec2() Vec2 {
	return Vec2{ f64(bytes.pop_f32()), f64(bytes.pop_f32()) }
}

pub fn (mut bytes PacketBytes) pop_vec3() Vec3 {
...
pub fn (mut bytes PacketBytes) pop_arr[T]() []T {
...
pub fn (mut bytes PacketBytes) push_u8(v u8) {
	bytes << v
}

pub fn (mut bytes PacketBytes) push_u16(v u16) {
	bytes << binary.big_endian_get_u16(v)
}

pub fn (mut bytes PacketBytes) push_u32(v u32) {
	bytes << binary.big_endian_get_u32(v)
}

pub fn (mut bytes PacketBytes) push_u64(v u64) {
	bytes << binary.big_endian_get_u64(v)
}

pub fn (mut bytes PacketBytes) push_i8(v i8) {
	bytes.push_u8(u8(v) - u8(min_i8))
}

pub fn (mut bytes PacketBytes) push_i16(v i16) {
	bytes.push_u16(u16(v) - u16(min_i16))
}

pub fn (mut bytes PacketBytes) push_i32(v i32) {
	bytes.push_u32(u32(v) - u32(min_i32))
}

pub fn (mut bytes PacketBytes) push_int(v int) {
	bytes.push_i32(int(v))
}

pub fn (mut bytes PacketBytes) push_i64(v i64) {
	bytes.push_u64(u64(v) - u64(min_i64))
}

pub fn (mut bytes PacketBytes) push_bool(v bool) {
	bytes << u8(v)
}

pub fn (mut bytes PacketBytes) push_f32(v f32) {
	bytes.push_u32(unsafe { U32_F32{f: v}.u })
}

pub fn (mut bytes PacketBytes) push_f64(v f64) {
	bytes.push_u64(unsafe { U64_F64{f: v}.u })
}

pub fn (mut bytes PacketBytes) push_string(v string) {
	bytes.push_arr[u8](v.bytes())
}

pub fn (mut bytes PacketBytes) push_color(v Color) {
	r, g, b, a := v.get_rgba8()
	bytes << [r, g, b, a]
}

pub fn (mut bytes PacketBytes) push_vec2(v Vec2) {
	bytes.push_f32(f32(v.x))
	bytes.push_f32(f32(v.y))
}

pub fn (mut bytes PacketBytes) push_vec3(v Vec3) {
	bytes.push_f32(f32(v.x))
	bytes.push_f32(f32(v.y))
...
// Note: Type T must be a supported type. Otherwise the length of the array will be popped, and an empty array of the given type with the given length will be returned, but no element will be popped.
pub fn (mut bytes PacketBytes) push_arr[T](arr []T) {
	len := int(arr.len)
